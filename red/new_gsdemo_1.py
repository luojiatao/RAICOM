...
coords_top_grap_myself = [56.5, 168.0, 320.0, 85.0, 40.7, -180.0] #[56.5, 170.0, 315.0, 86.53, 40.7, -178.72]
...
coords_bottom_grap_myself = [56.5, 168.0, 220.0, 84.53, 40.7, -180.0] #56.5, 170.0, 310.0, 86.53, 40.7, -178.72
...
coords_top_grap = [56.5, 151.0, 346.0, 83.53, 40.7, -180.0] #[56.5, 170.0, 315.0, 86.53, 40.7, -178.72]
...
coords_bottom_grap = [56.5, 151.0, 246.0, 83.53, 40.7, -180.0] #56.5, 170.0, 310.0, 86.53, 40.7, -178.72
...
y_bias = grabParams.y_bias1 
x_bias = grabParams.x_bias1 
...
class gsdemo(object):
    ...
    def pose_to_color(self):
        angles = [0, 0, 0, 0, 0, 0]
        self.mc.send_angles(angles,30)
        basic.grap(False)      
        time.sleep(1)         
        basic.move_to_target_coords(grabParams.coords_ready1,grabParams.GRAB_MOVE_SPEED)  

    def get_position_color(self, x, y):
        wx = wy = 0
        if grabParams.grab_direct == "front":
            wx = (self.c_y - y) * self.ratio1
            wy = (self.c_x - x) * self.ratio1
        elif grabParams.grab_direct == "right":
            wx = (self.c_x - x) * self.ratio1
            wy = (y - self.c_y) * self.ratio1
        return wx, wy
    ...
    def color_detect(self, img):
        ...
        for mycolor, item in self.HSV.items():
            ...
            if len(contours) > 0:
                ...
                boxes = [
                    box
                    for box in [cv.boundingRect(c) for c in contours]
                ...
                if boxes:
                    ...
                    cv.rectangle(img, (x, y), (x+w, y+h), (153, 153, 0), 2)
    ...
    def pick_and_place_color(self):
        ...
        while not done:
            ...
            detect_result = self.color_detect(img)
            self.show_image(img)
            if detect_result is None:
                continue
            else:
                x, y = detect_result
                real_x, real_y = self.get_position_color(x, y)
                coords_now = basic.get_coords()
                if len(coords_now) == 6:
                    coords = coords_now
                self.move_arm(real_x + x_bias, real_y + y_bias)
                self.place2right()
                time.sleep(2)
                angles2 = [-83.23, -140.53, 140.97, 58.71, -127.61, 5.71]
                self.mc.send_angles(angles2,25)
                time.sleep(2.5)
                done = True
                print("Done")
    ...
    def arm_back(self):
        angles1 = [0, 0, 0, 0, 0, 0]
        self.mc.send_angles(angles1,30)
        time.sleep(2)
